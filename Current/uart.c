...
char Transmiter_GetCharacterFromBuffer() {
   ...
   if((TRANSMITER_SIZE > sTransmiterBuffer.cCharCtr) && (NULL != sTransmiterBuffer.cData[sTransmiterBuffer.cCharCtr])) {
      cCharacter = sTransmiterBuffer.cData[sTransmiterBuffer.cCharCtr];
      sTransmiterBuffer.cCharCtr = sTransmiterBuffer.cCharCtr + 1;
   } else if(0 == sTransmiterBuffer.fLastCharacter) {
      cCharacter = TERMINATOR;
...
void UART_InitWithInt(unsigned int uiBaudRate) {
   ...
   U0IER |= mRX_DATA_AVALIABLE_INTERRUPT_ENABLE | mTHRE_INTERRUPT_ENABLE; // ustawianie przerwania, gdy sa dostpene dane do odczytu
